...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(134.0, 40.0),  # All numbers are approximate.
                rg.Point(122.0, 60.8),
                rg.Point(98.0, 60.8),
                rg.Point(86.0, 40.0),
                rg.Point(98.0, 19.2),
                rg.Point(122.0, 19.2)]
    circle = rg.Circle(rg.Point(110, 40), 24)
    answer = generate_points_on_circle(circle, 6)
...
def run_test_draw_points_on_circle():
    ...
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(150, 180), 100)
...
def run_test_pizza():
    ...
    title = 'PIZZA test 4:  75 slices, thin (thickness=2) orange lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 2
    pizza(window, circle, 75, 'orange', 3)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(pts)-1):
        line1 = rg.Line(pts[k], pts[k+1])
        line1.thickness = thickness
        line1.color = color
        line1.attach_to(window)
    line2 = rg.Line(pts[len(pts)-1], pts[0])
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4:  33 yellow lines on purple circle, ' \
            'hops = 12.'
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'purple'
    fancy_polygon(window, circle, 33, 12, 'yellow', 5)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(len(pts)):
        if (k + hops_to_next_point) > (len(pts) - 1):
            h = (k + hops_to_next_point) - len(pts)
        else:
            h = k + hops_to_next_point
